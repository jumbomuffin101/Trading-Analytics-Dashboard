...
// Compact KPI for Trades section (smaller type, no overflow)
...
export default function App() {
  ...
  const tradesWithDays = useMemo(() => {
    const t = (result?.trades ?? []).map(tr => ({ ...tr, days: daysBetween(tr.entry_date, tr.exit_date) }));
  ...
  const cumulativePnL = useMemo(() => {
    let acc = 0; return tradesWithDays.map(t => { acc += t.pnl; return { x:t.exit_date, y:acc }; });
  }, [tradesWithDays]);
    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-4 pt-6 pb-24">
        ...
        <div className="card p-7">
          ...
          {/* Disclaimer (subtle, not a banner) */}
          ...
          <div className="grid lg:grid-cols-3 gap-6 mt-6">
            ...
            <div className="card p-7">
              ...
              <div className="grid sm:grid-cols-3 gap-3 mb-4">
                ...
                <Kpi label="Avg Hold"    value={kpis.avgDays.toFixed(1) + " days"} tone="muted" />
              </div>
              ...
              <div className="max-h-[420px] overflow-auto">
                <table className="table table-fixed w-full text-sm">
                  <thead className="sticky top-0 bg-slate-900/80 backdrop-blur">
                    <tr>
                      <th className="w-[18%]">Entry</th>
                      <th className="w-[12%] text-right">Entry Px</th>
                      <th className="w-[18%]">Exit</th>
                      <th className="w-[12%] text-right">Exit Px</th>
                      <th className="w-[12%] text-right">PnL</th>
                      <th className="w-[10%] text-right">%</th>
                      <th className="w-[8%]  text-right">Days</th>
                  ...
                  <tfoot>
                    <tr className="bg-slate-900/40">
                      ...
                      <td className="text-right font-semibold">{signedPct(kpis.avgRet)}</td>
                      <td className="text-right font-semibold">{kpis.avgDays.toFixed(1)}</td>
