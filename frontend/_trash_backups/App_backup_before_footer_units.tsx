...
export default function App() {
  // yesterday ISO (avoid partial intraday)
  ...
  // helpers
  const signedMoney = (x:number) => (x>=0?"+":"") + x.toFixed(2);
  const signedPct   = (x:number) => (x>=0?"+":"") + (x*100).toFixed(2) + "%";

  // build a date->index map from price series so we can compute "bars held"
        ...
        <div className="relative mx-auto max-w-6xl px-4 pt-12 pb-14">
          ...
          <div className="mt-6 p-6 rounded-xl bg-slate-900/40 border border-slate-800 shadow-card">
            ...
            <p className="text-slate-300 leading-relaxed">
              ...
              the system goes long at today&apos;s close and exits after a fixed number of trading <em>bars</em>
              (trading days). One position at a time, with 100% equity sizing for compounding. We report per-trade outcomes,
        ...
        {result && (
          <div className="grid lg:grid-cols-3 gap-6 mt-6">
            ...
            <div className="card p-7">
              ...
              {/* KPI tiles: Best / Worst / Hold Period (parameter) */}
              <div className="grid sm:grid-cols-3 gap-3 mb-4">
                <Kpi label="Best Trade"  value={signedMoney(kpis.best)}  tone={kpis.best>=0 ? "bull" : "bear"} />
                <Kpi label="Worst Trade" value={signedMoney(kpis.worst)} tone={kpis.worst>=0 ? "bull" : "bear"} />
                ...
                <table className="table table-auto w-full text-sm">
                  <thead className="sticky top-0 bg-slate-900/80 backdrop-blur">
                    <tr>
                      ...
                      <th className="w-[12%] text-right cursor-pointer" onClick={()=>toggleSort("entry_date")}>Entry Px</th>
                    ...
                    {pagedTrades.map((t, i) => {
                      ...
                      return (
                        <tr key={i} className="hover:bg-slate-900/50">
                          ...
                          <td className={"text-right " + (positive ? "text-bull-400" : "text-bear-400")}>{signedMoney(t.pnl)}</td>
                          <td className={"text-right " + (positive ? "text-bull-400" : "text-bear-400")}>{signedPct(t.return_pct)}</td>
                          <td className="text-right">{Number.isFinite(t.daysBars) ? t.daysBars : "-"}</td>
                  ...
                  <tfoot>
                    <tr className="bg-slate-900/40">
                      ...
                      <td className="text-right font-semibold">{signedMoney(kpis.totalPnL)}</td>
                      <td className="text-right font-semibold">{(result?.metrics?.win_rate ?? 0)*100}%</td>
              ...
              {/* Paging controls only if needed */}
              {needPaging && (
