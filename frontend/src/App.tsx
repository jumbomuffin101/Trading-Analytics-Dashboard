...
function normalizePeek(raw: any) {
  ...
  const start = raw?.start ?? (preview.length ? preview[0].date : "") ?? "";
  const end =
    raw?.end ?? (preview.length ? preview[preview.length - 1].date : "") ?? "";
...
function normalizeBacktest(raw: any, reqBodyJson: any) {
  ...
  const initial_equity = n(
    pick(raw?.metrics?.initial_equity, raw?.equity_start, 1000)
  );
  ...
  const win_rate =
    n(pick(raw?.metrics?.win_rate, raw?.win_rate_pct, 0), 4) /
...
export default function App() {
  ...
  const startDefault = new Date(yday);
  startDefault.setDate(yday.getDate() - 120);
  ...
  const doBacktest = async () => {
    ...
    try {
      ...
      if (thr === null)
        throw new Error("Please enter a valid numeric threshold (try Peek).");
      if (hd === null)
        throw new Error("Hold Days must be a whole number ≥ 1.");
  ...
  const tradesWithBars = useMemo(() => {
    const t = (result?.trades ?? []).map((tr) => {
      ...
      const bars =
        iE !== undefined && iX !== undefined ? Math.max(0, iX - iE) : NaN;
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 pt-10 pb-6">
          <div className="flex items-center gap-3">
            <div className="h-11 w-11 rounded-xl bg-emerald-600 flex items-center justify-center font-black">
              $
            </div>
      ...
      <div className="mx-auto max-w-6xl px-4 pt-1 pb-10 space-y-8">
        ...
        <div className="card p-6 sm:p-7">
          ...
          <div className="text-xs text-slate-400 mt-1 mb-3">
            Pick a symbol, then choose dates and click Peek.
          </div>
          ...
          <div className="flex flex-wrap gap-3 mt-4 items-center">
            <button
              ...
              onClick={doPeek}
              disabled={loading || peekBusy || !canPeek}
            >
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-slate-400">
                  {fmtDate(peek.start)} – {fmtDate(peek.end)}
                </div>
              </div>
        ...
        {/* Strategy Parameters */}
        <div className="card p-6 sm:p-7">
          <h3 className="text-2xl font-bold tracking-tight text-emerald-400 mb-3">
            Strategy Parameters
          </h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4 text-[13px] leading-6">
              ...
              <ul className="list-disc ml-5 text-slate-300 space-y-1">
                <li>
                  <span className="font-medium">Threshold</span>: go long when
                  the close crosses <strong>above</strong> this price.
                </li>
                <li>
                  <span className="font-medium">Hold Days</span>: hold for N
                  trading days; exit at that day’s close.
                </li>
                <li>
                  One position at a time; P&amp;L realized on exits and added to
                  cash-only equity.
                </li>
              </ul>
            </div>
              ...
              <div className="card p-6 lg:col-span-2 flex flex-col">
                  ...
                  <ResponsiveContainer>
                    {mode === "equity" ? (
                      <AreaChart
                        ...
                        margin={{ left: 68, right: 16, top: 10, bottom: 38 }}
                      >
                        <defs>
                      ...
                      <LineChart
                        ...
                        margin={{ left: 72, right: 16, top: 10, bottom: 38 }}
                      >
                        <CartesianGrid
                          stroke="rgba(148,163,184,0.15)"
                          vertical={false}
                        ...
                        <ReferenceLine
                          y={
                            Number(result?.params?.threshold ?? threshold) ||
                            undefined
                          }
                          stroke="#f59e0b"
                          strokeDasharray="4 4"
                ...
                <div className="grid sm:grid-cols-4 gap-4 mt-5">
                  <Stat
                    ...
                    value={fmtSignedMoney2(result.metrics.total_pnl)}
                  />
                  <Stat
                    label="Win Rate"
                    value={fmtPct1(result.metrics.win_rate)}
                  />
                  <Stat
                    label="Annualized Return"
                    value={fmtPct2(result.metrics.annualized_return)}
                  />
                  <Stat
                    label="Trades"
                ...
                <div className="grid sm:grid-cols-4 gap-4 mt-4">
                  <Stat
                    label="Final Equity"
                    value={fmtMoney2(result.metrics.final_equity)}
                  />
                  <Stat
                    label="Max Drawdown"
                    value={fmtPct2(result.metrics.max_drawdown)}
                  />
                  <Stat
                    label="Average Trade Return"
                    value={fmtPct2(avgTradeReturn)}
                  />
                  <Stat
                    label="Initial Equity"
                    value={fmtMoney2(result.metrics.initial_equity)}
                  />
                </div>

                <div className="mt-3 text-xs text-slate-400 italic">
                  Equity starts at {fmtMoney2(result.metrics.initial_equity)} and
                  steps up/down only on exit days (one position at a time).
                </div>
              </div>
                      ...
                      <table className="table text-sm w-full">
                        <thead>
                          <tr>
                            ...
                            >
                              Date In {sortIndicator("entry_date")}
                            </th>
                            <th
                              className="cursor-pointer"
                              onClick={() => toggleSort("exit_date")}
                            >
                              Date Out {sortIndicator("exit_date")}
                            </th>
                            <th>Entry</th>
                            <th>Exit</th>
                            <th
                            ...
                            <th
                              className="cursor-pointer"
                              onClick={() => toggleSort("daysBars")}
                            >
                              Bars {sortIndicator("daysBars")}
                            </th>
                          </tr>
                        </thead>
                        <tbody>
...
function Stat({
  ...
  return (
    <div className="p-5 rounded-xl bg-slate-900/40 border border-slate-800">
      ...
      <div className="text-2xl font-semibold tabular-nums whitespace-nowrap leading-snug">
        {value}
      </div>
...
function Kpi({
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900/40 px-4 py-3 w-[180px] text-center">
      ...
      <div
        className={`text-lg font-semibold tabular-nums ${toneClass} whitespace-nowrap`}
      >
        {value}
      </div>
