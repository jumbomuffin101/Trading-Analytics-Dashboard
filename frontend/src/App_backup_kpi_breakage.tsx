...
function Kpi({ label, value, sub, tone }: { label: string; value: string; sub?: string; tone?: "bull" | "bear" | "muted" }) {
  const toneClass =
    tone === "bull" ? "text-emerald-300" :
    tone === "bear" ? "text-rose-300" :
    "text-slate-200";
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-4 min-w-0">
      <div className="text-xs text-slate-400 truncate">{label}</div>
      <div className={"text-2xl font-semibold leading-7 tabular-nums truncate " + toneClass}>{value}</div>
          ...
          <div className="grid lg:grid-cols-3 gap-6 mt-6">
            ...
            <div className="card p-7">
              ...
              <div className="grid sm:grid-cols-3 gap-3 mb-4">
                <Kpi label="Best Trade"  value={fmtSignedMoney2(kpis.best)}  tone={kpis.best>=0 ? "bull" : "bear"} sub="USD" />=0?"+":"") + kpis.best.toFixed(2)}  tone={kpis.best>=0 ? "bull" : "bear"} />
                <Kpi label="Worst Trade" value={fmtSignedMoney2(kpis.worst)} tone={kpis.worst>=0 ? "bull" : "bear"} sub="USD" />=0?"+":"") + kpis.worst.toFixed(2)} tone={kpis.worst>=0 ? "bull" : "bear"} />
